...
from segmentation_model import CityScapesDataLoader, Model
...
event_acc22 = EventAccumulator('lightning_logs/version_13/events.out.tfevents.1651087015.dhcp-10-250-196-68.harvard.edu.789.0')
...
def show_iou():
    ...
    print(blur_vals + blur_vals1)
    plt.plot(x, sharp_vals + sharp_vals1)
    print(sharp_vals + sharp_vals1)
...
def show_predictions():
    ...
    test_class = CityScapesDataLoader('data/evaluation_base', split="val", mode='fine', target_type='semantic', transforms=transform)
    model = Model.load_from_checkpoint('models20/standard.ckpt')
    ...
    fig.suptitle('Predictions from Standard Dataset Model')
    ax[0].axis('off')
    ax[1].axis('off')
    ax[2].axis('off')
    plt.show()
...
def calculate_iou(data, model_name):
    import numpy as np
    ...
    test_class = CityScapesDataLoader('data/' + data, split="val", mode='fine', target_type='semantic', transforms=transform)
    model = Model.load_from_checkpoint('models20/' + model_name + '.ckpt')

    test_loader=DataLoader(test_class, batch_size=250, shuffle=False)
...
def show_eval_ious():
    x_standard = [0.71, 0.61, 0.39]
    x_sharp = [0.72, 0.59, 0.37]
    x_blur = [0.66, 0.56, 0.38]
    y = [0,1,2]
    # print(base_vals, blur_vals, sharp_vals)
    plt.plot(y, x_standard)
    plt.plot(y, x_sharp)
    plt.plot(y, x_blur)
    values = ['Standard', 'Grayscale', 'Stylized']
    # plt.plot(x, blur_vals)
    # plt.plot(x, sharp_vals)
    plt.legend(['Standard Cityscapes', 'Sharpened Cityscapes', 'Blurred Cityscapes'])
    plt.title('Intersection Over Union by Model by Test Dataset')
    plt.xlabel('Dataset')
...
# iou = calculate_iou('evaluation_stylized', 'standard')
# print(iou)
#
# iou = calculate_iou('evaluation_stylized', 'sharp')
# print(iou)# create_maxps()
#
# iou = calculate_iou('evaluation_stylized', 'blur')
# print(iou)
# # show_predictions()
show_eval_ious()